...
for (pheno in c("diff", "log_diff", "prop")){
    df[paste0("pheno_", pheno)] = residuals(lm(paste0("pheno_", pheno, "_out ~ BMI*sex + age_drug + drug_start*dose + post_measure_DT + ", 
...
pheno = "diff"
df[paste0("pheno_", pheno, "_adj")] = residuals(lm(paste0("pheno_", pheno, "_out ~ baseline_measure + BMI*sex + age_drug + drug_start*dose + post_measure_DT + ", 
                                    covs_filter, paste0("PC", 1:20, collapse="+")),
                                    data = df, na.action = na.exclude))
