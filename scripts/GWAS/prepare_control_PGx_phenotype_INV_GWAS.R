...
for (pheno in c("diff", "log_diff", "prop")){
    ...
    df[paste0("pheno_", pheno)] = residuals(lm(paste0("pheno_inv ~ sex + age_m1 + measure_DT + ", paste0("PC", 1:20, collapse="+")),
...
pheno = "diff"
df$pheno_inv = qnorm((rank(df[paste0("pheno_", pheno, "_out")], na.last="keep")-0.5)/sum(!is.na(df[paste0("pheno_", pheno, "_out")])))
df[paste0("pheno_", pheno, "_adj")] = residuals(lm(paste0("pheno_inv ~ measure1 + sex + age_m1 + measure_DT + ", paste0("PC", 1:20, collapse="+")),
                                    data = df, na.action = na.exclude))
