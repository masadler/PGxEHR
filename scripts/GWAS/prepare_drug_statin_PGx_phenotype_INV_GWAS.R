...
for (pheno in c("diff", "log_diff", "prop")){
    ...
    df[paste0("pheno_", pheno)] = residuals(lm(paste0("pheno_inv ~ sex + age_drug + drug_start*dose + post_measure_DT + ", 
...
pheno = "diff"
df$pheno_inv = qnorm((rank(df[paste0("pheno_", pheno, "_out")], na.last="keep")-0.5)/sum(!is.na(df[paste0("pheno_", pheno, "_out")])))
df[paste0("pheno_", pheno, "_adj")] = residuals(lm(paste0("pheno_inv ~ baseline_measure + sex + age_drug + drug_start*dose + post_measure_DT + ", 
                                    covs_filter, paste0("PC", 1:20, collapse="+")),
                                    data = df, na.action = na.exclude))
