...
for (pair in medication_measure_pairs){
  ...
  for (filter in c("s", "l")){
    ...
    if ((pair == "antihpt_CCB_SBP") & (filter == "s")) next
    ...
    for (n_meas in c("1", "m")){
        ...
        for (out_pheno in c("pheno_diff", "pheno_log_diff", "pheno_prop", "pheno_diff_adj")){
            ...
            X_all_sub <- fread(paste0("output/GWAS/drug_", pair, "_", filter, "_", n_meas, "_GWASformatted_", out_pheno, ".tsv"))
            ...
            title <- paste0(drug, "-", phenotype, ", ", pheno_name, " (", variant, ", ", measure, " meas.); N = ", N)
            ...
            png(paste0("output/Manhattan/drug_", pair, "_", filter, "_", n_meas, "_GWAS_", out_pheno, "_manhattan.png"), width = 8, height = 5, res = 300, units = "in")
            print(p_manhattan)
            dev.off()
        }
    }
