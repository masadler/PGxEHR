...
"""
Extract metformin drug response phenotypes
...
dt_no_main = 365 # time before which no antidiabetic prescription should be present
...
# impute dose with median if missing (there should be none missing)
medication_df["is_metformin"] = ~medication_df.drug.isna()
median_dose = medication_df[medication_df.is_metformin == True]['dose'].median()
medication_df = medication_df.rename(columns = {"dose": "dose_original"})
medication_df['dose'] = medication_df.apply(lambda row: 
                                               median_dose
                                               if (pd.isnull(row['dose_original']) & row.is_metformin & (row['combi'] == "no"))
                                               else row['dose_original'], axis=1)
...
def med_qc(df, time, drug, dose, post_times, filter, sulfonylureas_prior):
    ...
    for post_time in post_times:
        ...
        if df_post.shape[0] < 1:
            ...
            return qc, compl, dose_avg, reason, max_post_time, sulfonylureas_during

        # prescriptions between initiation and post-measure
        df_during = df[df.issue_date < post_time]

        if filter == "s":
            if sum(df_during["category"] != "biguanides") > 0:
                if post_time != first_post_time:
                    break
                qc = False
                reason = "antidiab. therapy change during treat."
                return qc, compl, dose_avg, reason, max_post_time, sulfonylureas_during
            ...
            # QC5: no medication changes - if prior sulfonylureas then ok if sulfonylureas during (no possibility for drug change)
            if sum(df_during["category"] != "biguanides") > 0:
                # combinations have category "other"
                if ((sum(~df_during["category"].isin(["sulfonylureas", "biguanides"])) > 0)) | ((sum(df_during["category"] == "sulfonylureas") > 0) & (sulfonylureas_prior == "no")):
                    if post_time != first_post_time:
                        break
                    qc = False
                    reason = "antidiab. therapy change during treat."
                ...
                if ((sum(df_during["category"] == "sulfonylureas") > 0) & (sulfonylureas_prior == "yes")):
                    sulfonylureas_during = "yes"
    ...
    if filter == "l":
        df = df[df.issue_date < max_post_time]
        df = df[(df.drug == drug)] # there could be sulfonylureas
...
def PGx_phenotype(medication_df, measure_df, primary_data_df, filter, n_meas, baseline_dt_before, baseline_dt_after, post_dt_min, post_dt_max, prim_care_time):
    ...
    keys = ["eid", "drug_start_date", "drug_start", "dose_start", "dose_avg", "baseline_measure", "baseline_DT", "post_measure", "post_measure_DT", "completeness", "sulfonylureas_prior", "sulfonylureas_during"]
    ...
    for eid in medication_df.eid.unique():
        ...
        if medication_person_df_nonessential.shape[0] > 0:
            ...
            medication_person_df_nonessential_sulf = medication_person_df_nonessential[medication_person_df_nonessential.category == "sulfonylureas"]
            medication_person_df_nonessential_other = medication_person_df_nonessential[medication_person_df_nonessential.category == "other"]

            if medication_person_df_nonessential_sulf[(medication_person_df_nonessential_sulf.issue_date <= (start_time)) & 
                                                      (medication_person_df_nonessential_sulf.issue_date  > (start_time - timedelta(days=dt_no_main)))].shape[0] > 0:
     
                sulfonylureas_prior = "yes"

                if filter == "s":
                    removed["eid"].append(eid)
                    removed["reason"].append("prior antidiabetic use")
                    continue
                else:
        ...
        qc, compl, dose_avg, reason, max_post_time, sulfonylureas_during = med_qc(medication_person_df, start_time, drug_start_info["drug"], drug_start_info["dose"], post_times, filter, sulfonylureas_prior)
        ...
        result["sulfonylureas_prior"].append(sulfonylureas_prior)
        result["sulfonylureas_during"].append(sulfonylureas_during)
