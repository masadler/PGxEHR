...
m = snakemake.wildcards["measure"] # HR
...
min = 30
max = 200
codes = ["242..", "X773r", "X773s", "242Z.", "UKBB"]
...
dt_no_main = 365 # time before which no betablocker prescription should be present
...
# impute dose with median of betablocker type if missing 
medication_df["is_betablocker"] = ~medication_df.drug.isna()
median_dose_by_type = medication_df[medication_df.is_betablocker == True].groupby('drug')['dose'].median()
...
medication_df['dose'] = medication_df.apply(lambda row: 
                                               ...
                                               if (pd.isnull(row['dose_original']) & row.is_betablocker & (row['combi'] == "no"))
...
def PGx_phenotype(medication_df, measure_df, primary_data_df, filter, n_meas, baseline_dt_before, baseline_dt_after, post_dt_min, post_dt_max, prim_care_time):
    ...
    keys = ["eid", "drug_start_date", "drug_start", "dose_start", "dose_avg", "baseline_measure", "baseline_DT", "post_measure", "post_measure_DT", "completeness"]
    ...
    for eid in medication_df.eid.unique():
        ...
        if medication_person_df_essential.shape[0] < 1: # no betablocker at all
            removed["eid"].append(eid)
        ...
        # QC3 # there should be no medication other than betablockers, but maybe drug not defined
        if medication_person_df_nonessential.shape[0] > 0:
            ...
            if medication_person_df_nonessential[(medication_person_df_nonessential.issue_date <= (start_time)) & 
                ...
                removed["eid"].append(eid)
                removed["reason"].append("prior undefined betablocker use")
                continue
