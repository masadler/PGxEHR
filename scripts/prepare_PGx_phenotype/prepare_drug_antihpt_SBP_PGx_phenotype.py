...
"""
Extract betablocker drug response phenotypes
...
#### antihpt medication class
if antihpt_class == ["all"]:
    antihpt_class = ["ACEi", "thiazide_diuretics", "CCB", "ARB", "alpha_agonist", "alpha_blocker", "vasodilator", "renin-i"]

#### biomarker measures filtering parameters

min = 50
max = 300
codes = ["2469.", "246..", "2464.", "2465.", "2466.", "2467.", "246K.", "UKBB"]
...
dt_no_main = 365 # time before which no other antihpt prescription should be present
...
def med_qc(df, time, drug, dose, type, post_times, filter):
    
    qc = True
    compl = 0
    reason = ""
    bblocker_during = "no"
    loop_diuretics_during = "no"
    ...
    for post_time in post_times:
        ...
        if df_post.shape[0] < 1:
            ...
            return qc, compl, dose_avg, reason, bblocker_during, loop_diuretics_during, max_post_time

        # prescriptions between initiation and post-measure
        df_during = df[df.issue_date < post_time]

        # QC5: check for medication changes - treat bblocker and loop diuretics exceptions (if lenient)
        if (sum(~df_during["type"].isin(antihpt_class)) > 0) & (filter == "l"):
            if ((sum(df_during["type"] == "betablocker") > 0)):
                bblocker_during = "yes"
            if ((sum(df_during["type"] == "loop_diuretics") > 0)):
                loop_diuretics_during = "yes"
                        
            if type != "betablocker":
                df_during = df_during[(df_during.type != "betablocker") & (df_during.type != "loop_diuretics")] # remove potential betablocker and loop diuretics entries
            else:
                df_during = df_during[(df_during.type != "loop_diuretics")]

        if type == "betablocker":
            if sum(df_during["bb_combi"] != "no") > 0:
                if post_time != first_post_time:
                    break
                qc = False
    ...
    if type != "betablocker":
        df = df[(df.type != "betablocker") & (df.type != "loop_diuretics")] # remove potential betablocker and loop diuretics entries
    else:
        df = df[(df.type != "loop_diuretics")]
...
def PGx_phenotype(medication_df, measure_df, primary_data_df, filter, n_meas, baseline_dt_before, baseline_dt_after, post_dt_min, post_dt_max, prim_care_time):
    ...
    keys = ["eid", "drug_start_date", "drug_start", "dose_start", "type_start", "dose_avg", "baseline_measure", "baseline_DT", 
            "post_measure", "post_measure_DT", "completeness", "bblocker_prior", "loop_diuretics_prior", "bblocker_during", "loop_diuretics_during"]
    ...
    for eid in medication_df.eid.unique():
        ...
        medication_person_df_nonessential = medication_person_df[~medication_person_df.type.isin(antihpt_class)]
        ...
        type_start = drug_start_info["type"]

        if type_start == "betablocker":
            if drug_start_info["bb_combi"] != "no":
                removed["eid"].append(eid)
                removed["reason"].append("combination medication")
                continue

            # QC3
        if medication_person_df_nonessential.shape[0] > 1:
            medication_person_df_nonessential = medication_person_df_nonessential.sort_values("issue_date")
            medication_person_df_nonessential_bb = medication_person_df_nonessential[medication_person_df_nonessential.type == "betablocker"]
            medication_person_df_nonessential_ld = medication_person_df_nonessential[medication_person_df_nonessential.type == "loop_diuretics"]
            medication_person_df_nonessential_other = medication_person_df_nonessential[(medication_person_df_nonessential.type != "betablocker") & 
                                                                                        (medication_person_df_nonessential.type != "loop_diuretics")]

            if medication_person_df_nonessential_bb[(medication_person_df_nonessential_bb.issue_date <= (start_time)) & 
                                                    (medication_person_df_nonessential_bb.issue_date  > (start_time - timedelta(days=dt_no_main)))].shape[0] > 0:
                bblocker_prior = "yes"
                if filter == "s":
                    removed["eid"].append(eid)
                    removed["reason"].append("prior antihpt use")
                    continue

            if medication_person_df_nonessential_ld[(medication_person_df_nonessential_ld.issue_date <= (start_time)) & 
                                                    (medication_person_df_nonessential_ld.issue_date  > (start_time - timedelta(days=dt_no_main)))].shape[0] > 0:
                loop_diuretics_prior = "yes"
                if filter == "s":
                    removed["eid"].append(eid)
                    removed["reason"].append("prior antihpt use")
                    continue

            if medication_person_df_nonessential_other[(medication_person_df_nonessential_other.issue_date <= (start_time)) & 
                                                    (medication_person_df_nonessential_other.issue_date  > (start_time - timedelta(days=dt_no_main)))].shape[0] > 0:
                removed["eid"].append(eid)
                removed["reason"].append("prior antihpt use")
                continue
        ...
        qc, compl, dose_avg, reason, bblocker_during, loop_diuretics_during, max_post_time = med_qc(medication_person_df, start_time, drug_start_info["drug"], drug_start_info["dose"], type_start, post_times, filter)
        ...
        if filter == "l":
            if loop_diuretics_prior != loop_diuretics_during:
                removed["eid"].append(eid)
                removed["reason"].append("no consistent loop diuretics intake")
                continue
            if (type_start != "betablocker") & (bblocker_prior != bblocker_during):
                removed["eid"].append(eid)
                removed["reason"].append("no consistent betablocker intake")
                continue
        ...
        result["type_start"].append(drug_start_info["type"])
        result["dose_start"].append(drug_start_info["dose"])
        result["dose_avg"].append(dose_avg)
        result["completeness"].append(compl)
        result["bblocker_prior"].append(bblocker_prior)
        result["bblocker_during"].append(bblocker_during)
        result["loop_diuretics_prior"].append(loop_diuretics_prior)
        result["loop_diuretics_during"].append(loop_diuretics_during)
